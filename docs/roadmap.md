...
- A 5-minute quickstart that takes a developer from `pip install resurf` to a passing task run.
...
3. **Prompt-driven task generation (BYOK LLM).** A `resurf task generate` command that, given an NL goal and live-site introspection, drafts a task YAML grounded in seeded product/user data, with a draft success predicate the user reviews before saving.
...
5. **Native Node SDK.** Today the SDK is Python-only; Node teams use the Stagehand bridge. A first-class `@resurf/sdk` Node package would close that gap.
