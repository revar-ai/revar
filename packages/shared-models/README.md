...
Shared SQLModel schema and deterministic seed data for revar sites.

This package is imported by both the site backend (FastAPI) and the revar SDK so that:
...
Each site has its own subpackage (e.g. `revar_models.shop_v1`).
